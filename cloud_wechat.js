...
AV.Cloud.define('fetchWeChatOpenId', async (request, response) => {
  ...
  try {
    ...
    const res = await axios.post(`${config.rest_url}/users`, data).then(res=>{
      console.log("res###################",res)
      console.log("res.body###################",res.body)
    })
    response.success(res.data); 
    const id = res.data.id;
    const updateRea = axios.post(`${config.acf_url}/users/${id}`, { fields: { wechatopenid: openid, accumulatedmbincent: 10000 } });
